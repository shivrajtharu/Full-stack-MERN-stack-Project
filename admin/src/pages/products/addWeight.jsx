...
const AddWeight = () => {
  ...
  const fetchWeight = async () => {
    try {
      ...
      if (res?.success) {
        ...
        context.notify("Failed to fetch Product Weight list", "error");
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formFields.name.trim()) {
      context.notify("Please enter product Weight name", "error");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        res = await editData(
          `/api/product/updateProductWeight/${editingId}`,
          formFields
        );
      } else {
        res = await postData("/api/product/productWeight/create", formFields);
      }
      ...
      if (res?.success) {
        ...
        context.notify(
          editingId
            ? "Product Weight updated successfully"
            : "Product Weight created successfully",
          "success"
        );
        fetchWeight();
      } else {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await deleteData(`/api/product/productWeight/${id}`);
      if (res?.success) {
        context.notify("Product Weight deleted", "success");
        fetchWeight();
      } else {
        context.notify("Failed to delete Product Weight", "error");
      }
    } catch (error) {
  ...
  const handleDeleteMultiple = async () => {
    try {
      const res = await deleteMultipleData(`/api/product/deleteMultipleWeight`, {
        ids: selectedIds,
      });

      if (res?.success) {
        context.notify("Selected Weight deleted", "success");
        setSelectedIds([]);
        fetchWeight();
      } else {
        context.notify("Failed to delete selected Weight", "error");
      }
    } catch (err) {
      context.notify("Error deleting selected Weight", "error");
    }
  };

  const editWeight = async (id) => {
    try {
      ...
      if (res?.success) {
        ...
        context.notify("Failed to load Weight data", "error");
      }
    } catch (error) {
      context.notify("Error loading Weight data", "error");
    }
  };
